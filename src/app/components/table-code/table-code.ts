...
@Component({
  ...
  imports: [FormsModule, MonacoEditorModule, MatTabsModule],
  templateUrl: './table-code.html',
  styleUrl: './table-code.css',
})
export class TableCode {
  @ViewChild('meuIframe') iframeRef!: ElementRef<HTMLIFrameElement>;

  htmlCode = signal(`<label class="container">
  <div class="simple-text">Clique!</div>
...
`);
  cssCode = signal(`.container input {
  position: absolute;
  opacity: 0;
  cursor: pointer;
  height: 0;
...
`);
  jsCode = signal('console.log("olá, mundo!")');
  ...
  renderizarPreview() {
    console.log('Tentando renderizar...', this.iframeRef);
    if (!this.iframeRef) {
      console.error('Iframe ainda não está pronto!');
      return;
    }
